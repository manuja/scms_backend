		...
		if($beneval_fund_req==1){
			...
			if($user_id>0){
				...
				if($paymentsummeryforreceipt[0]->main_payment_category_id==2){
					...
					$subjectemail="ANNUAL IESL MEMBERSHIP SUBSCRIPTIONS – ".$years_title[0]->year;
...
function getSubsReminderNoticeMsg($salutation,$mem_details,$year,$member_ship_number,$beneval_fund_amount){
        				...
        				$msg_body.='<h3><font color="red">ANNUAL IESL MEMBERSHIP SUBSCRIPTIONS – '.$year.'</font></h3><br>';

        				$msg_body.='Kindly be notified that, your IESL Membership Subscription payment for the Year '.$year.' is due on 01st January '.$year.'.</br></br>';

        				if($current_date<=$discount_end_date){

        					$msg_body.='<b>1.</b> A discount of 10% will apply to all categories of Members (excluding Students), who pay IESL subscription fees for the year '.$year.' on or before 31st January '.$year.'. The related invoice (with 10% discount) is indicated below.</br></br>';  
        			...
        			//	$msg_body.='The related invoice is indicated below. Kindly settle the payment at your earliest to get registered with the IESL for year '.$year.'.</br></br>'; 
...
//$msg_body.='A separate invoice will be sent to you for the Engineering Council Sri Lanka (ECSL) registration. <br><br>';
						...
						$Click='<a href="https://iesl.lk/mis/">Click here</a>';
						$msg_body.='<h3><font color="red">Please </font> '.$Click.' (for Online payments)
</h3> <br>';

						$msg_body.='If you have already paid your subscription for '.$year.', please ignore this notice. For any clarifications in this regard, please contact Mr Praneeth Costa (email: praneeth@iesl.lk) with a copy to the Finance Manager (email: mgr.fin@iesl.lk).</br></br>';
						
						$msg_body.='<b>Kind regards,</b> <br><br>';
						$msg_body.='<b>Chief Executive Officer/Executive Secretary</b> <br><br>';
						$msg_body.='<b>IESL</b> <br><br>';
                ...
                $msg_body.='<ul><li> a) <b><u>Online payment</u></b> by using the IESL MIS system which is the easiest and simplest way. Please click the link,  <br>';

                	$Click='<a href="https://iesl.lk/mis/">https://iesl.lk/mis/</a>';
						$msg_body.='<h3>'.$Click.'</h3></li>';

			 $msg_body.='<li> b) <b><u>Online banking</u></b> to the Institution of Engineers, Sri Lanka Account No: 0002323113 maintained at the Torrington Square Branch of the Bank of Ceylon. The confirmation of payment received from the bank has to be sent  via email to praneeth@iesl.lk  indicating  your membership number and full name. </li><br>';
  ...
  //$msg_body.='<li>d) Cheques drawn in favor of The Institution of Engineers, Sri Lanka and posted to IESL.</li><br>';
    //$msg_body.='<li>e) Cash/Cheque /Credit Card/Debit card payments made in person during normal working hours(8.30 am to 4.30 pm)at IESL Headquarters, Wijerama Mawatha, Colombo 7.</li></ul>';
...
function getECSLReminderNoticeMsg($salutation,$mem_details,$year,$member_ship_number){
        				...
        				$msg_body.='If you are NOT serving as an engineering practitioner in Sri Lanka, please inform our Finance Manager in writing (email: mgr.fin@iesl.lk) with a copy to DCEO/DES (email: des.mem@iesl.lk) for us to de-list you from being invoiced in the future. A record of IESL members who declared that they do not serve as engineering practitioners in Sri Lanka, will be sent to ECSL periodically. For further details on Engineering Council registration, please refer the link,</br>'; 

    

    $Click='<a href="https://iesl.lk/index.php?option=com_content&view=article&id=169&Itemid=388&lang=en">https://iesl.lk/index.php?option=com_content&view=article&id=169&Itemid=388&lang=en</a>';
						$msg_body.='<h3>'.$Click.'
</h3> <br>';
   ...
   public function  RemovedPendingInvoice($master_table_name, $master_table_record_id){
        ...
        $today = date('Y-m-d');
         ...
         $this->CI->db->trans_start();
	  		...
	  		$remarks="Invoices removed by IESL on ".$today;
	     	$dataw = array(
	     	'cancel_status'=> 2,
            'state' =>-2,
            'cancel_by' =>$user_id,
